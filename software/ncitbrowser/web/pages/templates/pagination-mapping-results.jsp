...
<FORM
    NAME="paginationForm"
    METHOD="POST"
    action="<%=request.getContextPath() %>/pages/mapping_search_results.jsf?nav_type=mappings">
    ...
    <tr>
      ...
      <td class="textbody" align=right>
        ...
        <% if (page_num > 1) { %> &nbsp;
          <i>
            <a
                href="<%=request.getContextPath() %>/pages/mapping_search_results.jsf?dictionary=<%=mapping_scheme%>&page_number=<%=prev_page_num_str%>">
        ...
        if (num_pages > 1) {
          ...
          for (int idx=sliding_window_start; idx<=sliding_window_end; idx++) {
            ...
            if (page_num != idx) {
              %>
              <a
                  href="<%=request.getContextPath() %>/pages/mapping_search_results.jsf?dictionary=<%=mapping_scheme%>&page_number=<%=idx_str%>">
        ...
        if (num_pages > 1 && next_page_num <= num_pages) {
          ...
          <i>
            <a
                href="<%=request.getContextPath() %>/pages/mapping_search_results.jsf?dictionary=<%=mapping_scheme%>&page_number=<%=next_page_num_str%>">
      ...
      <td class="textbody" align=left>
        ...
        <select name=resultsPerPage size=1 onChange="paginationForm.submit();">
          ...
          if (resultsPerPageValues == null) {
            try {
              String error_msg = "WARNING: The server encountered an unexpected error (file: pagination-mapping-results.jsp, code: 1, var: resultsPerPageValues).";
