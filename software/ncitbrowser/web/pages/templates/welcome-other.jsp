  ...
  <table border="0" width="700px" role='presentation'>
    <tr>
      ...
      if (isMapping) {
        ...
        <td align="right">
          <a
              href="<%=request.getContextPath() %>/pages/mapping.jsf?dictionary=<%=HTTPUtils.cleanXSS(scheme)%>&version=<%=vocablary_version_value%>">

            <img src="<%=basePath%>/images/ViewMapping.gif" alt="View Mapping" style="border-style:none;" />
          </a>
          &nbsp;
          <a
              href="<%=request.getContextPath() %>/ajax?action=export_mapping&dictionary=<%=HTTPUtils.cleanXSS(scheme)%>&version=<%=vocablary_version_value%>">
    ...
    <tr>
      <td class="textbody" width="388px" valign="top" align="left">
        <% if (DataUtils.isNullOrBlank(scheme)) { %>
          <p class="textbodyred">
            &nbsp;
            <b><%= Constants.UNIDENTIFIABLE_VOCABULARY %></b>
          </p>
        <% } else if (html_compatable_description_value == null) { %><%= HTTPUtils.cleanXSS(scheme) %>        <% } else { %>
          ...
          <tr valign="top">
            ...
            <td>
              <a href="<%= request.getContextPath() %>/start.jsf" alt="NCI Term Browser">
                <img
                    src="<%=basePath%>/images/EVSTermsBrowserTile.gif"
                    width="77"
                    height="38px"
