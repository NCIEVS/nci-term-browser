...
<table width="900px" cellspacing="0" cellpadding="0" border="0" class="tabTable" role='presentation'>
  <tr>
    <%
    ...
    <td width="134" height="21">
      ...
      <% } else if (data_type.compareTo("properties") != 0) { %>
        <a
            href="<%=request.getContextPath() %>/pages/<%=jsp_page_name%>?dictionary=<%=scheme%>&version=<%=tab_version%>&code=<%=id%>&ns=<%=_ns%>&type=properties&key=<%=_search_key%>&b=1&n=<%=_page_number%>&vse=<%=_vse%>"">
    ...
    <td width="119" height="21">
      ...
      (data_type != null && data_type.compareTo("synonym") != 0)) {
        %>
        <a
            href="<%=request.getContextPath() %>/pages/<%=jsp_page_name%>?dictionary=<%=scheme%>&version=<%=tab_version%>&code=<%=id%>&ns=<%=_ns%>&type=synonym&key=<%=_search_key%>&b=1&n=<%=_page_number%>&vse=<%=_vse%>"">
    ...
    <td width="102" height="21">
      ...
      (data_type != null && data_type.compareTo("relationship") != 0)) {
        %>
        <a
            href="<%=request.getContextPath() %>/pages/<%=jsp_page_name%>?dictionary=<%=scheme%>&version=<%=tab_version%>&code=<%=id%>&ns=<%=_ns%>&type=relationship&key=<%=_search_key%>&b=1&n=<%=_page_number%>&vse=<%=_vse%>"">
    ...
    <% if (!typeLink_isMapping) { %>
      <td width="85" height="21">
        ...
        (data_type != null && data_type.compareTo("mapping") != 0)) {
          %>
          <a
              href="<%=request.getContextPath() %>/pages/<%=jsp_page_name%>?dictionary=<%=scheme%>&version=<%=tab_version%>&code=<%=id%>&ns=<%=_ns%>&type=mapping&key=<%=_search_key%>&b=1&n=<%=_page_number%>&vse=<%=_vse%>"">
    ...
    <td width="71" height="21">
      ...
      (data_type != null && data_type.compareTo("all") != 0)) {
        %>
        <a
            href="<%=request.getContextPath() %>/pages/<%=jsp_page_name%>?dictionary=<%=scheme%>&version=<%=tab_version%>&code=<%=id%>&ns=<%=_ns%>&type=all&key=<%=_search_key%>&b=1&n=<%=_page_number%>&vse=<%=_vse%>"">
